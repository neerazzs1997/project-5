...
const updateProduct = async function (req, res) {
    try {
        ...
        if (!validator.isValidBody(data)) {
            if (!(validator.isValidBody(req.files)))
            ...
            if (!validator.isValidPrice(price)) {
              return res
                ...
                .send({
                  status: false,
                  message:
                    "Bad request please provoide valid price only in digits",
                });
            }
        ...
        if (req.files) {
            ...
            if (files && files.length > 0) {
                var uploadedFileURL = await awsUrl.uploadFile(files[0])
            }else{
...
const createProduct = async function (req, res) {
    try {
        ...
        return res.status(500).send({ status: false, msg: err.message, Success:"successfull" })
    }
}
...
//         const {description, title, price, style, installments } = requestBody
...
//             // // CONVERT AVAILABLE INTO JSON PARSE
//         let availableSizes
//         console.log(availableSizes)
          

//             if(!validator.isValid(requestBody.availableSizes)){
//                 return res.status(400).send({status:false, message:"Bad request please provoide valid availableSizes"})
//             }

//             availableSizes = JSON.parse(requestBody.availableSizes)
...
//      const updatedProduct = await productModel.findByIdAndUpdate(productId, requestBody , {new:true})
